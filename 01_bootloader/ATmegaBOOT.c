...
// 马尔代夫主义者，网络上似乎也没找到什么介绍。可能是比较散漫，享受于慢节奏的一批人？
/* We, Malmoitians, like slow interaction
 * therefore the slow baud rate ;-)
 */
// 查看相关的信息，F_CPU是1M，这样，下面的这个参数其实是500K
...
// sfr是特殊功能寄存器的意思，下面这个outb应该是给特殊寄存器按照一个字节进行赋值。
// inb应该是按照字节的方式读寄存器
#ifndef outb
...
// 下面的两组命令应该是位操作，cbi应该是清除指定寄存器的bit，sbi应该是设置指定寄存器的bit
#ifndef cbi
...
// 这个注释看上去跟实际的代码不是很相符，从接口的名称看，下面是EEPROM的操作。
// 三个功能分别是：读一个字节、读一个word、写一个字节。
// 感想：这个读写居然这么直接，本以为可能会有一系列复杂的flash驱动。这里居然是对地址的直接操作。
...
int main(void)
{
    ...
    /* m8 */
    // 初始化UART，这里有些信息没找到定义，应该需要从其他地方获取到。
    ...
    for (;;)
    {
        ...
        // ASCII中，14是设备控制4,10是行转意
        // 如果收到了1，看看是否带空格。如果带，恢复下面一串字符串
        else if (ch == '1')
        {
            if (getch() == ' ')
            {
        ...
        // 如果收到@，看看后面的字符是否大于0x85。如果大于0x85，再接收下一个字节。
        // 看上去，这个判断不是按照ASCII来的，因为已经超过了其范围。
        else if (ch == '@')
        {
            ch2 = getch();
            if (ch2 > 0x85)
        ...
        {
            // 接收20个字节，但是没有存储的buffer？
            // 从注释看，这里似乎是想实现一个简单的延时
            getNch(20);
            nothing_response();
        }

        /* Parallel programming stuff  DON'T CARE  */
        // 接收到E之后，一定时间的延时后，回复无消息
        else if (ch == 'E')
        {
            getNch(5);
            nothing_response();
        ...
        // 接收到P之后，回复无消息。不过，从注释上看，这里应该是进入编程模式。这里并没看到相应的处理。
        else if (ch == 'P')
        {
            nothing_response();
        ...
        else if (ch == 'd')
        {
            ...
            if (getch() == 'E')
                flags.eeprom = 1;
            // 读取数据，存到BUF，但是这里可能溢出。因为BUF大小256
            ...
            if (getch() == ' ')
            {
                ...
                { //Write to FLASH one page at a time
                    ...
                    // FLASH的编程，采用的汇编操作。\n\t的确是指令格式
                    // 具体的实现，可以跳过。
                    // 有一点需要注意到：这个烧写过程中的多字节信息，是在汇编的循环中实现的。
                    asm volatile(
                        "clr	r17		\n\t"     //page_word_count
...
{
    // 先获取一个空格字符成功，然后输出数值。前后分别有一个14 10
    if (getch() == ' ')
    {
        putch(0x14);
        putch(val);
...
// 如果收到的字符是0，执行这个接口。如果继续收到空格，恢复0x14 0x10
// 如果没有收到空格，直接跳过。
void nothing_response(void)
{
    if (getch() == ' ')
